<script context="module">
  import { getRandomIndex } from "../utils/web3";
  ...
  export function getRandomDuck(length, upperBoundPerType) {
    let randArr = [];
  ...
  export function getRandomDuckOnChain(seed, length, upperBoundPerType) {
    let nextRandomIndex = getRandomIndex(seed);
    let randArr = [];
    // one type can only have limit cards
    let typeCounts = [0, 0, 0, 0];
    for (let i = 0; i < length; i++) {
      let ranIdx = nextRandomIndex(pookers.length);
      if (
        randArr.indexOf(pookers[ranIdx]) == -1 &&
        !pookers[ranIdx].chosen &&
        typeCounts[pookers[ranIdx].type] < upperBoundPerType
